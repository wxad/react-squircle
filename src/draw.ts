...
export function getPathParamsForCorner({
  ...
  let p = (1 + cornerSmoothing) * radius;
  ...
  // But what this means is that at some point when radius is large enough,
...
function drawTopRightPath({
  ...
  if (radius) {
    return rounded`
    c ${a} 0 ${a + b} 0 ${a + b + c} ${d}
    a ${radius} ${radius} 0 0 1 ${arcSectionLength} ${arcSectionLength}
    c ${d} ${c}
        ${d} ${b + c}
        ${d} ${a + b + c}`;
  } else {
